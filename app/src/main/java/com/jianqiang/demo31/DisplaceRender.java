...
public class DisplaceRender implements GLSurfaceView.Renderer {
    private VaryTools tools;
    private Cube cube;
    private Context mContext;

    public DisplaceRender(Context context){
        tools = new VaryTools();
        cube = new Cube();
        mContext = context;
    }
...
//最终得到的矩阵
        ...
        tools.frustum(-ratio, ratio, -1,1,3, 20);
        //存储生成矩阵元素的float[]类型数组
        //填充起始偏移量
        //摄像机位置X,Y,Z坐标
        //观察目标X,Y,Z坐标
        ...
        tools.setCamera(5,5,10,0,0,0,0,1,0);
//以上两个方法只能得到矩阵并不能使其生效
        //下面通过矩阵计算得到最终想要的矩阵
        //存放结果的总变换矩阵
        //结果矩阵偏移量
    ...
    public void onDrawFrame(GL10 gl) {
        ...
        mMVPMatrix = tools.getFinalMatrix();
